...
# Spasi "aneh" yang dipakai model bahasa. Diuji 12 Sep: jawaban Groq menulis
# "Jakarta Kota" dengan non-breaking space, dan tombol "Buka Jakarta Kota"
# tidak pernah muncul walau namanya jelas tertulis di jawaban. Bugnya tidak
# kelihatan di layar karena NBSP tampil persis seperti spasi biasa.
#
# Penggantinya WAJIB sepanjang satu karakter juga: posisi hasil pencocokan
# dipakai untuk mengurutkan tombol dan mendeteksi nama yang saling tumpang
# tindih, jadi indeksnya tidak boleh bergeser.
SPASI_ANEH = re.compile(
    r"[\u00a0\u1680\u2000-\u200b\u202f\u205f\u3000\ufeff]"
)
...
def susun_aksi(
    ...
    for entri in log:
        ...
        elif alat == "peluang_sponsorship":
            for p in hasil.get("peluang", [])[:2]:
                buka(p["stasiun"], tab="Ad-Space")
    ...
    # Stasiun yang SUDAH punya tombol - termasuk tombol yang membuka tab tertentu
    # - dilewati: "Ad-Space Cawang" dan "Buka Cawang" berdampingan hanya menambah
    # pilihan tanpa menambah kemampuan.
    sudah = {a["station_id"] for a in aksi if a["jenis"] == "buka_stasiun"}
    for sid in _nama_di_teks(reply, peta)[:MAKS_STASIUN_DARI_TEKS]:
        if sid != station_id_konteks and sid not in sudah:
