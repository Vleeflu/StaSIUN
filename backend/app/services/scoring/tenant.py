...
def compute_category(
    ...
    if not any(r["supply"] for r in rows):
        raise TenantError(
            ...
            f"{sumber!r}. Kemungkinan besar kategorinya memang tidak ada di lajur "
            f"itu, bukan benar-benar tidak ada pesaingnya. Kategori tenant berasal "
            f"dari nama layer GEO MAPID, jadi lajurnya harus 'mapid'."
        )
