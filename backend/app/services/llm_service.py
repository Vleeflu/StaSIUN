...
from openai import APIError, AsyncOpenAI, RateLimitError  # noqa: F401

from app.services.llm_penyedia import PanggilanGagal, Rantai, SemuaPenyediaHabis
...
class LLMService:
    ...
    def _rantai(self) -> Rantai:
        """Rantai penyedia untuk satu pertanyaan.

        Dibuat per pertanyaan, bukan sekali seumur proses: kuota harian penyedia
        pertama bisa pulih di antara dua pertanyaan, dan asisten sebaiknya
        kembali ke penyedia utama begitu itu terjadi.
        """
        return Rantai()
    ...
    async def chat(
        ...
        if db is not None:
            return await self._chat_dengan_alat(self._rantai(), messages, db)

        try:
            response = await self._rantai().panggil_async(messages=messages)
        except SemuaPenyediaHabis as e:
            raise RuntimeError(
                "Seluruh penyedia model sedang kehabisan kuota harian. "
                "Tambahkan penyedia cadangan lewat LLM_FALLBACKS di .env."
            ) from e
        except RateLimitError as e:
            raise RuntimeError(
                "Kuota penyedia model sedang habis, coba lagi sebentar."
            ) from e
        except PanggilanGagal as e:
            # Penyedianya disebut dari objek galatnya sendiri, BUKAN dari
            # settings. Versi sebelumnya membaca `settings.LLM_BASE_URL` -
            # penyedia utama - sehingga galat dari Gemini dilaporkan sebagai
            # galat Groq, dan penelusurannya menunjuk ke arah yang salah.
            raise RuntimeError(f"Panggilan ke model gagal: {e}") from e
    ...
    async def _chat_dengan_alat(
        self, rantai: Rantai, messages: list[dict], db
    ) -> tuple[str, list[dict]]:
        ...
        for _ in range(MAX_PUTARAN_ALAT):
            try:
                response = await rantai.panggil_async(
